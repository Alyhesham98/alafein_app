    ...
    try {
      ...
      if (isCategory) {
        ...
        response = await client.get(
          Uri.parse(
              'https://alafein.azurewebsites.net/api/v1/Event/GetPagination?PageNumber=${SessionManagement.pageNumber}&PageSize=${SessionManagement.pageSize}'),
          headers: {
            "Authorization": "Bearer ${SessionManagement.getUserToken()}"
          },
        );
      ...
      if (response.statusCode == 200) {
        ...
        for (int i = 0; i < eventsData.length; i++) {
          ...
          ListEventModel event =
              ListEventModel.fromJson(eventsData[i]); // as Map<String, dynamic>
          events.add(event);
        }
        return events;
      } else {
